...
public class SuccessorStringGenerator {
	...
	public static String next(String text) {
		...
		if (!alphaNum || alphaNumPos == 0 || alphaNumPos == len) {
			...
			// Strip the input text for non alpha numeric prefix. We do not need to process
			// these prefix but to save and
	...
	/**
	 * Internal method to calculate string successor value on alpha numeric chars
	 * only.
	 */
	private static void next(StringBuilder buf, int pos, boolean alphaNum) {
		...
		if (Character.isDigit(c)) {
			...
			// If input text has any alpha num at all then we are to calc next these
			// characters only and ignore the
			// we will do this by recursively call into next char in buf.
			if (alphaNum) {
				next(buf, pos - 1, alphaNum);
			} else {
				// However if the entire input text is non alpha numeric, then we will calc
				// successor by simply
